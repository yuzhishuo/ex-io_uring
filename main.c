...
#include <sys/uio.h>   // for writev, readv and struct iovec
#include <sys/stat.h>  // for stat
#include <linux/fs.h>  // for struct iovec
...
off_t get_file_size(int fd)
{
    ...
    if (fstat(fd, &st) < 0) //  from <sys/stat.h>    {
    {
        perror("fstat");
        return -1;
    }
    if (S_ISBLK(st.st_mode)) // from <sys/stat.h>
    {
...
{
    struct iovec *iovecs;
    int file_fd = open(file_name, O_RDONLY); // from fcntl.h
    if (file_fd < 0)
    {
        perror("open");
        return 1;
    ...
    if (file_sz % BLOCK_SZ)
        blocks++;
    iovecs = malloc(sizeof(struct iovec) * blocks); // from stdlib.h
