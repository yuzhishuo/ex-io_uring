...
static int setup_buffer_pool(struct ctx *ctx) {
  ...
  for (i = 0; i < BUFFERS; i++) {
    io_uring_buf_ring_add(ctx->buf_ring, get_buffer(ctx, i), buffer_size(ctx),
...
static int setup_context(struct ctx *ctx) {
  ...
  params.flags =
      IORING_SETUP_SUBMIT_ALL | IORING_SETUP_COOP_TASKRUN | IORING_SETUP_CQSIZE;
...
static int setup_sock(int af, int port) {
  ...
  if (af == AF_INET6) {
    struct sockaddr_in6 addr6 = {
        .sin6_family = af, .sin6_port = nport, .sin6_addr = IN6ADDR_ANY_INIT};
...
static void recycle_buffer(struct ctx *ctx, int idx) {
  io_uring_buf_ring_add(ctx->buf_ring, get_buffer(ctx, idx), buffer_size(ctx),
                        idx, io_uring_buf_ring_mask(BUFFERS), 0);
...
static int process_cqe_recv(struct ctx *ctx, struct io_uring_cqe *cqe,
  ...
  o = io_uring_recvmsg_validate(get_buffer(ctx, cqe->flags >> 16), cqe->res,
                                &ctx->msg);
  if (!o) {
  ...
  if (ctx->verbose) {
    ...
    fprintf(stderr, "received %u bytes %d from [%s]:%d\n",
            io_uring_recvmsg_payload_length(o, cqe->res, &ctx->msg), o->namelen,
            name, (int)ntohs(addr->sin_port));
  }
