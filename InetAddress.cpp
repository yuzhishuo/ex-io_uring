...
InetAddress::InetAddress(uint16_t portArg, bool loopbackOnly, bool ipv6) {
  ...
  if (ipv6) {
    ...
    addr6_.sin6_addr = ip;
    addr6_.sin6_port = endian::host2network(portArg);
  } else {
    bzero(&addr_, sizeof addr_);
    addr_.sin_family = AF_INET;
    in_addr_t ip = loopbackOnly ? kInaddrLoopback : kInaddrAny;
    addr_.sin_addr.s_addr = endian::host2network(ip);
    addr_.sin_port = endian::host2network(portArg);
  }
}
...
std::string InetAddress::toIpPort() const {
  char buf[64] = "";
  ye::toIpPort(buf, sizeof buf, getSockAddr());
  return buf;
}

std::string InetAddress::toIp() const {
  char buf[64] = "";
  ye::toIp(buf, sizeof buf, getSockAddr());
  return buf;
}
...
uint16_t InetAddress::port() const {
  return endian::network2host(portNetEndian());
}
...
bool InetAddress::resolve(std::string_view hostname, InetAddress *out) {
  ...
  if (ret == 0 && he != NULL) {
    ...
    if (ret) {
      spdlog::error("InetAddress::resolve");
    }
    return false;
  }
}
